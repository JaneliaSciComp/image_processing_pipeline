class Settings:
   defaultFileBase = '/groups/lightsheet/lightsheet/home/ackermand/Lightsheet-Processing-Pipeline/Compiled_Functions/sampleInput_'
   outputDirectoryBase = '/groups/lightsheet/lightsheet/home/ackermand/interface_output/'
   lightsheetProcessing = 'http://10.36.13.18:9000/api/rest-v2/async-services/lightsheetProcessing'
   # lightSheetDB = 'mongodb://dev-mongodb:27017/lightsheet'
   lightSheetDB = 'mongodb://localhost:27017'
   username = 'lightsheet'